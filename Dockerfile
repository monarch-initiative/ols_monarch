...
COPY ols/ols-config.yaml ${OLS_HOME}
COPY ols/logo-monarch.jpg ${OLS_HOME}/web-custom/
...
COPY ols/application.properties ${OLS_HOME}
COPY src ${OLS_HOME}/src
COPY Makefile ${OLS_HOME}
RUN ls -l ${OLS_HOME}
...
CMD cd ${OLS_HOME} && cat ols-config.yaml \
